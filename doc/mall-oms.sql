...
CREATE TABLE `oms_order` (
  ...
  `transaction_id` varchar(64) DEFAULT NULL COMMENT '交易编号',
  ...
  `pay_type` int(1) DEFAULT 0 COMMENT '支付方式：0->未支付；1->支付宝；2->微信',
  `source_type` int(1) DEFAULT 1 COMMENT '订单来源：0->PC订单；1->app订单',
  `status` int(1) DEFAULT 0 COMMENT '订单状态：0->待付款；1->待发货；2->已发货；3->已完成；4->已关闭；5->无效订单',
  `order_type` int(1) DEFAULT 0 COMMENT '订单类型：0->正常订单；1->秒杀订单',
  `delivery_code` varchar(64) DEFAULT NULL COMMENT '物流公司编码',
  `delivery_company` varchar(64) DEFAULT NULL COMMENT '物流公司名称',
...
CREATE TABLE `oms_order_item` (
  ...
  `order_sn` varchar(64) NOT NULL COMMENT '订单编号',
  `return_id` bigint(20) DEFAULT NULL COMMENT '退货id',
  `product_id` bigint(20) NOT NULL,
...
CREATE TABLE `oms_order_return_apply` (
  ...
  `product_id` bigint(20) NOT NULL COMMENT '退货商品id',
  `order_sn` varchar(64) NOT NULL COMMENT '订单编号',
  `member_id` bigint(20) NOT NULL,
  ...
  `delivery_code` varchar(100) DEFAULT NULL COMMENT '退货快递公司',
  `delivery_sn` varchar(100) DEFAULT NULL COMMENT '退货货运单号',
  ...
  `create_time` datetime DEFAULT NOW() COMMENT '申请时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `oms_order_setting` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `flash_order_overtime` int(11) NOT NULL COMMENT '秒杀订单超时关闭时间(分)',
  `normal_order_overtime` int(11) NOT NULL COMMENT '正常订单超时时间(分)',
  `confirm_overtime` int(11) NOT NULL COMMENT '发货后自动确认收货时间（天）',
  `finish_overtime` int(11) NOT NULL COMMENT '自动完成交易时间，不能申请售后（天）',
  `comment_overtime` int(11) NOT NULL COMMENT '订单完成后自动好评时间（天）',
  PRIMARY KEY (`id`)
